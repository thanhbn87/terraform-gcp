...
resource "google_compute_instance_group" "group_01" {
  count       = "${length(compact(concat(data.null_data_source.instance_lists.*.inputs.self_links_01))) > 0 ? 1 : 0}"
  name        = "${var.name}-01"
  description = "${var.desc} group 01"

  instances = ["${compact(concat(data.null_data_source.instance_lists.*.inputs.self_links_01))}"]
